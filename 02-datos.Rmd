...
```{r drivers-gdal-table, echo=FALSE, eval=is_html(), fig.cap="Listado de drivers en la instalación de GDAL`."}
...
* `r citepkg("giscoR", "https://ec.europa.eu/eurostat/web/gisco")`: permite importar datos de [Eurostat - GISCO]() (*Geographic Information System of the COmmission*). 
...
```{r osmdata, eval=.regerar, fig.path="images/", dev="png", fig.cap="Representación de las carreteras, calles y caminos en A Coruña (generado con el paquete `osmdata`)."}
library(osmdata) 
...
```
`r latexfig("osmdata")`
```{r osmdata-plot, echo=FALSE, eval=!.regerar, fig.cap="Representación de las carreteras, calles y caminos en A Coruña (generado con el paquete `osmdata`)."}
...
El paquete `r citepkg("gstat", "https://r-spatial.github.io/gstat")` permite la modelización geoestadística (univariante, Capítulo 3, y multivariante, Capítulo 5), espacial y espacio-temporal (Capítulo 6), incluyendo predicción y simulación (Capítulo 4 y secciones 5.X y 6.X). 
